  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(
          "mode",
          itinerariesList[0].children.slice(2)[0].attributes.mode
        );
        let tripLegs = itinerariesList[0].children.slice(2).map((leg) => {
          return {
            ...
            fromLat: leg
              ...
              .getElementsByTagName("lat")[0].value,
            fromLon: leg
              .getElementsByTagName("from")[0]
              .getElementsByTagName("lon")[0].value,
            destLat: leg
        ...
        <Card.Content className="card-content-area card-description">
          <List ordered divided>
            {itineraries.map((leg, index) => (
              <List.Item key={index}>
                <p>
                  {leg.mode} {leg.duration} minutes | {leg.distance}mi
                </p>
              </List.Item>
            ))}
          </List>
